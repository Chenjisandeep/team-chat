...
export default function RegisterPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setMessage("Registered successfully! Redirecting to channels...");
      ...
      <div className="w-full max-w-md rounded-3xl border border-zinc-800 bg-zinc-900/70 shadow-2xl backdrop-blur-md p-8 space-y-6">
        <header className="space-y-1">
          ...
          <p className="text-xs text-zinc-500">
            Join channels and chat in real time with your team.
          </p>
        </header>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {message && (
            <p className="text-xs text-emerald-300 bg-emerald-950/30 border border-emerald-800 rounded-xl px-3 py-2">
              {message}
            </p>
          )}
